...
class RatioInterpreter:
    ...
    def __init__(self, industry: str = "general"):
        ...
        self.benchmarks = self.BENCHMARKS.get(
            self.industry, self._get_general_benchmarks()
        )
        ...
        if ratio_name in self.benchmarks:
            ...
            if ratio_name in ["current_ratio", "roe", "gross_margin"]:
                ...
                if value >= benchmark["excellent"]:
                    ...
                    interpretation["message"] = (
                        "Below industry standards - attention needed"
                    )
                ...
                if value > 0:
                    if value < benchmark["undervalued"]:
                        ...
                        interpretation["message"] = (
                            "Market pricing in growth expectations"
                        )
                    else:
...
def perform_comprehensive_analysis(
    ...
    for category, category_ratios in ratios.items():
        ...
        for ratio_name, value in category_ratios.items():
            if isinstance(value, (int, float)):
                analysis["current_analysis"][category][ratio_name] = (
                    interpreter.interpret_ratio(ratio_name, value)
                )
